...
void Player::Delete_Player()
{
	...
	ifstream file1("basket.txt", ios_base::in);																				// зчитуванн€
	while (!file1.eof())																									//даних гравц≥в  
	{																														//
		file1 >> player[i].name >> player[i].name1 >> player[i].nationality>> player[i].club_name >>player[i].age;			//
		file1 >> player[i].position >> player[i].weight >> player[i].height >> player[i].price;								//≥з файлу
		i++;																												//
	}																														//
	file1.close();
	...
	for(i=0;i<=max;i++)
	{
		if((who==player[i].name)&&(who1==player[i].name1)) { tmp=i; break;}      // пошук гравц€ €кого потр≥бно видалити
	...
	for (i = 0; i<=MAX; ++i)
	{
		...
		if(tmp==i)
		{
			cout<<"\nDeleted!\n"; break;                                                   // запис даних у файл без "видаленого" гравц€
		}
		else
		{
...
void Player::Edit_Player()
{
	...
	//пошук гравц€ в €кого треба зм≥нити дан≥
	M1:	cout<<"\nEnter name of player or press [1] [space] [1] to exit: "<<endl; cin>>who; cin>>who1;
	if(who=="1") {return;}
	int max=0, k=0, MAX;
	max=i-1;
	...
	// виб≥р ≥ редагуванн€ необх≥дного пол€
...
//функц≥€ що покузаЇ виконан≥  трансфери
void Function::view_transfer()
{
	...
	//зчитуванн€ ≥ виведенн€ на екран даних ≥з файлу
	cout<<'\n'<<lin_e1<<'\n'<<hedtr<<'\n'<<lin_e1<<'\n';
	while(!file.eof())
	{
...
void Function::Transfer()
{
	...
	function.Get_Data();  //виведенн€ вс≥х гравц≥в на екран
...
M1:	cout<<"\nEnter name of player or press [1] [space] [1] to exit: "<<endl; cin>>who; cin>>who1;
	...
	for(i=0;i<=max;i++)
	{
		if((who==player[i].name)&&(who1==player[i].name1)) { tmp_player=i; break;}  //пошук гравц€
		if((who!=player[i].name)&&(who1!=player[i].name1))
		{
		k++;
		}
	...
	//зчитуванн€ ≥з файлу даних про клуби
...
M2:	cout<<"\nEnter name of club or press [1] to exit: "<<endl; cin>>where;
	...
	for(j=0;j<=max;j++)
	{
		if(club[j].club_name==player[tmp_player].club_name)
		{
			...
			SUM1=atoi(player[tmp_player].price.c_str());
			SUM+=SUM1;          //додаванн€ кошт≥в до бюджету клубу зв≥дки переходить гравець
			TMP=j;
		}
	}
	
	from=player[tmp_player].club_name;
	player[tmp_player].club_name=club[tmp_club].club_name;   //зм≥на клубу
...
M1:	cout<<"\n   If you are Admin press [1];\n   If you are User press [2];\n\n   If you want to exit press [3];\n   ";
	...
	if(choice==1)
	{
		cout<<"   Please enter the password: "; cin>>pass; //перев≥рка паролю
...
//вив≥д даних про гравц≥в на екран
...
M1:	cout<<"Please enter on which value you want to search: \n[1] Name\n[2] Surname\n[3] Nationality\n[4] Club\n[5]";
	cout<<" Age\n[6] Position\n[7] Weight\n[8] Height\n[9] Price\n[10] Exit\n\n";   //введенн€ значенн€ по €кому буде проводитись пошук
	cin>>z;
	if ((z < 1)||(z > 10)||(!cin)) 
	...
	switch(z)
	{
		case 1: {	cout<<"Enter name: "<<endl;
					cin>>X;
					function.Search_Player(player,&Player::name,X,1); //виклик функц≥њ пошуку перший аргумент це вказанн€ структури, 
																	//другий - за €ким параметром проводитьс€ пошук, трет≥й - шукане значенн€,  
					break;}											//четвертий: 1 - шукане значенн€ буквене, 2 - циферне	
...
void Player::Search_Player(Player player[], string Player::*where, string what, int variant)
{
	...
	if (!file)
	{
		return;    //перрев≥рка чи в≥дкритий файл
	}
	else
		//зчитуванн€ даних ≥з файлу
		while (!file.eof())
		...
		for (i = 0; i<max; i++)
			...
			if ( (player[i].*where) == what )  // пор≥внюванн€ параметра ≥з шуканим значенн€м
				{
				cout <<n<<". " << player[i].name<< " " <<  player[i].name1 << setw(6)<<'\t' << '|' << '\t' <<  player[i].nationality <<setw(7)<< '\t' << '|' << '\t';
				cout<<  player[i].club_name << setw(8)<<'\t' << '|' << '\t' <<  player[i].age << '\t' << '|' << '\t' <<   player[i].position << '\t' << '|' << '\t';
		...
		case 2: {
			...
			{
			 tmp=first_limit;
			 first_limit=second_limit; //перестановка меж, в раз≥ €кщо перше значенн€ б≥льше другого
			 second_limit=tmp;
		    }
...
//виведенн€ списку клуб≥в на екран
void Club::Show_Club()
{
	Club club[size2];
	int i=0, n=1;
